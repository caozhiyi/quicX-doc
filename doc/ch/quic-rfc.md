...
+ Token Length：指定令牌字段长度的可变长度整数，单位为字节。如果不存在标记，则此值为零。服务器发送的初始包必须将Token Length字段设置为零；接收具有非零Token Length字段的初始数据包的客户端必须丢弃该数据包，或者生成类型为**PROTOCOL_VIOLATION**的连接错误。    
...
+ Retry Token：服务器可用于验证客户端地址的不透明令牌。
...
客户端必须丢弃具有无法验证的重试完整性标记的重试数据包，请参阅[QUIC-TLS]的“重试数据包完整性”部分。这会降低非路径攻击者注入重试数据包的能力，并防止重试数据包意外损坏。客户端必须丢弃具有零长度重试令牌字段的重试数据包。    
客户端使用包含所提供的重试令牌的初始数据包响应重试数据包，以继续建立连接。    
客户端将此初始数据包的目标连接ID字段设置为重试数据包中源连接ID的值。更改初始数据包ID也会导致更改目的地中的连接密钥。它还将Token字段设置为重试中提供的令牌。客户端不能更改源连接ID，因为服务器可以将连接ID作为其令牌验证逻辑的一部分（请参阅第8.1.4节）。    
来自客户端的下一个初始数据包使用来自重试数据包的连接ID和令牌值（参见第7.2节）。除此之外，由客户机发送的初始分组与第一初始分组受相同的限制。客户机必须使用它包含在此数据包中的相同加密握手消息。服务器可以将包含不同加密握手消息的数据包视为连接错误或丢弃它。    
...
+ stateless_reset_token (0x02): 无状态重置令牌用于验证无状态重置；请参阅第10.4节。此参数是一个16字节的序列。此传输参数不能由客户端发送，但可以由服务器发送。不发送此传输参数的服务器不能对握手期间协商的连接ID使用无状态重置（第10.4节）。
...
客户机不能包含仅用于服务器的传输参数（原始的连接标识、无状态的重置令牌或首选的地址）。服务器必须将接收到这些传输参数中的任何一个视为**TRANSPORT_PARAMETER_ERROR**类型的连接错误。    
