...
除非应用程序指定的优先级另有指示（见第2.3节），否则端点应优先考虑数据的重传而不是发送新数据。    
...
## 13.4 显式拥塞通知
QUIC端点可以使用显式拥塞通知（ECN）[RFC3168]来检测和响应网络拥塞，ECN允许网络节点通过在包的IP报头中设置码位而不是丢弃它来指示网络中的拥塞。如[QUIC-RECOVERY]所述，端点通过降低响应发送速率来应对拥塞。    
为了使用ECN，QUIC端点首先确定路径是否支持ECN标记，并且对等方是否能够访问IP报头中的ECN码位。如果ECN标记的数据包被丢弃或ECN标记被重写，则网络路径不支持ECN。端点验证在建立连接期间和迁移到新路径时在路径上使用ECN（第9节）。

## 13.4.1 ECN计数
当接收到带有ECT或CE码点的QUIC包时，可以从封闭IP包访问ECN码点的启用ECN的端点增加相应的ECT（0）、ECT（1）或CE计数，并将这些计数包括在随后的ACK帧中（见第13.2节和第19.3节）。注意，这需要能够从封闭的IP包中读取ECN代码点，这在所有平台上是不可能的。    
接收器检测到的重复数据包不会影响接收器的本地ECN码位计数；有关安全问题，请参阅（第21.8节）。
如果一个端点接收到一个在IP包报头中没有ECT或CE码位的QUIC包，它按照第13.2节的要求用ACK帧响应，而不增加任何ECN计数。如果一个端点没有实现ECN支持或者不能访问接收到的ECN代码点，它不会增加ECN计数。    
...
### 13.4.2 ECN验证
有故障的网络设备有可能损坏或错误地丢弃带有ECN标记的数据包。为了在这种设备存在的情况下提供健壮的连接，每个端点独立地验证ECN计数，并在检测到错误时禁用ECN。    
端点独立验证每个网络路径上发送的数据包的ECN。因此，端点在建立新连接、切换到新的服务器首选地址时以及在活动连接迁移到新路径时验证ECN。附录B描述了一种测试ECN支持路径的可能算法。    
即使一个端点没有在它传输的数据包上使用ECN标记，该端点也必须提供关于从对等方接收到的ECN标记的反馈（如果这些标记是可访问的）。未能报告ECN计数将导致对等方禁用ECN标记。

### 13.4.2.1 发送ECN标记
要启动ECN验证，端点在向对等方发送新路径上的数据包时应执行以下操作：
+ 在新路径上发送到对等端的早期传出数据包的IP报头中设置ECT（0）码位[RFC8311]。
+ 如果使用ECT（0）码位发送的所有数据包最终被视为丢失[QUIC-RECOVERY]，则验证被视为失败。    
为了减少将拥塞性丢失误认为是由故障网络元件丢弃的数据包的可能性，端点可以在路径上的前十个传出数据包中设置ECT（0）码位，或者在三个RTT周期内设置ECT（0），以先发生的为准。    
实现可以试验和使用其他策略来使用ECN。其他探测ECN支持路径的方法也是可能的，不同的标记策略也是如此。实现也可以使用ECT（1）代码点，如[RFC8311]中所述。

### 13.4.2.2 接收ACK帧
在其传输的数据包上设置ECT（0）或ECT（1）码位的端点必须在接收ACK帧时使用以下步骤来验证ECN。
+ 如果该ACK帧新确认了端点发送的、设置了ECT（0）或ECT（1）码位的包，并且如果ACK帧中没有ECN反馈，则验证失败。这一步可以防止将ECN位置零的网络元素和无法访问ECN标记的对等方，因为在这些情况下，对等方可以在没有ECN反馈的情况下作出响应。
...
处理ECN计数时出错可能会导致验证失败。如果该ACK帧没有提前该连接中确认的最大分组号，则端点不应执行该验证。    
当ACK帧丢失时，端点可能会错过包的确认。因此，ECT（0）、ECT（1）和CE计数的总增加可能大于ACK帧中确认的分组的数目。当这种情况发生时，如果验证成功，则必须增加本地引用计数以匹配ACK帧中的计数。

### 13.4.2.3 验证结果
如果验证失败，那么端点将停止在随后的IP包中发送ECN标记，期望网络路径或对等方不支持ECN。验证成功后，端点可以继续在后续数据包中设置ECT码位，并期望路径是ECN能力的。但是，网络路由和路径元素可以更改中间连接；如果在连接中的任何点验证失败，则端点必须禁用ECN。    
即使验证失败，端点也可以在连接的任何晚些时候在同一路径上重新验证ECN。
...
终结点应该使用数据报打包层PMTU Discovery（[DPLPMTUD]）或实现路径MTU Discovery（PMTUD）[rfc191][RFC8201]来确定到目标的路径是否支持所需的消息大小而无碎片。    
在没有这些机制的情况下，QUIC端点不应该发送大于1280字节的IP包。假设最小的IP报头大小，这将导致QUIC最大数据包大小对于IPv6为1232字节，对于IPv4为1252字节。QUIC实现在计算QUIC最大包大小时可能更加保守，以允许未知的隧道开销或IP报头选项/扩展。    
每对本地和远程地址可以有不同的PMTU。因此，实现任何类型的PMTU发现的QUIC实现应该为本地和远程IP地址的每个组合保持最大包大小。    
如果QUIC端点确定任何一对本地和远程IP地址之间的PMTU低于支持允许的最小最大数据包大小所需的大小，则它必须立即停止在受影响路径上发送QUIC数据包（PMTU探测数据包除外）。如果找不到替代路径，端点可以终止连接。
...
端点必须忽略声称PMTU已减少到1280字节以下的ICMP消息。    
生成ICMP的要求（[RFC1812]，[RFC4443]）规定，引用的数据包应包含尽可能多的原始数据包，而不超过IP版本的最小MTU。如[DPLPMTUD]第1.1节所述，被引用的包的大小实际上可以更小，或者信息无法理解。    
QUIC端点应按照[RFC8201]和[RFC8085]第5.2节的规定验证ICMP消息，以防止出现非路径注入。此验证应使用ICMP消息有效负载中提供的带引号的数据包，将该消息与相应的传输连接[DPLPMTUD]相关联。    
ICMP消息验证必须包括匹配的IP地址和UDP端口[RFC8085]，并且在可能的情况下，包括到活动QUIC会话的连接id。     
还可以提供进一步的验证：
+ IPv4端点可以在一小部分数据包上设置Don't Fragment（DF）位，以便大多数无效的ICMP消息在没有DF包未完成的情况下到达，因此可以将其标识为伪消息。
+ 端点可以存储来自IP或UDP报头的附加信息以用于验证（例如，IP ID或UDP校验和）。
终结点应忽略验证失败的所有ICMP消息。    
终结点不能基于ICMP消息增加PMTU。在QUIC的丢失检测算法确定引用的包实际上已经丢失之前，QUIC最大包大小的任何减少都可能是临时的。
...
依赖目的地连接ID来路由QUIC包的端点可能需要将连接ID包括在PMTU探测包中，以便将任何产生的ICMP消息（第14.2节）路由回正确的端点。但是，只有长头数据包（17.2节）包含源连接ID，并且一旦握手完成，长头数据包不会被对等方解密或确认。构造PMTU探测器的一种方法是将握手包（见第12.2节）与单个UDP数据报中的短报头包合并（见第17.2.4节）。如果UDP数据报到达端点，握手包将被忽略，但是短头数据包将被确认。如果UDP数据报引出一个ICMP消息，则该消息可能包含UDP数据报的引用部分内的源连接ID。
...
发送方必须使用能够表示比最大确认包和发送的包号之间的差值大两倍的包号大小。接收包的对等方随后将正确地解码分组号，除非分组在传输过程中被延迟，使得它在接收到多个编号更高的分组之后到达。端点应该使用足够大的数据包编号编码，以允许恢复数据包编号，即使数据包在随后发送的数据包之后到达。结果，分组号编码的大小至少比包括新分组的连续未确认分组号的数目的以2为底的对数多1位。    
例如，如果一个端点接收到对分组0xabe8bc的确认，那么发送一个编号为0xac5c02的分组需要16位或更多的分组号编码；而发送一个编号为0xace8fe的分组需要24位分组号编码。    
...
+ DCID Len：版本后面的字节包含其后面的目标连接ID字段的长度（以字节为单位）。此长度编码为8位无符号整数。在QUIC版本1中，此值不得超过20。接收值大于20的版本1长标头的终结点必须丢弃该数据包。服务器应该能够从其他QUIC版本读取更长的连接id，以便正确地形成版本协商包。
+ 目的连接ID：目的连接ID字段跟在DCID Len后面，长度在0到20个字节之间。第7.2节详细描述了该字段的使用。
+ SCID Len：目标连接ID后面的字节包含它后面的源连接ID字段的长度（以字节为单位）。此长度编码为8位无符号整数。在QUIC版本1中，此值不得超过20字节。接收值大于20的版本1长标头的终结点必须丢弃该数据包。服务器应该能够从其他QUIC版本读取更长的连接id，以便正确地形成版本协商包。    
...
+ 保留位（R）：字节0的两个位（掩码为0x0c）跨多个数据包类型保留。这些位使用头保护进行保护（见[QUIC-TLS]第5.4节）。保护前包含的值必须设置为0。端点必须在移除封包和标头保护后，将接收到的这些位元具有非零值的封包，视为**PROTOCOL_VIOLATION**类型的连线错误。在删除头保护之后丢弃这样的包会使端点暴露于攻击之下（参见[QUIC-TLS]第9.3节）。
...
初始分组的有效载荷包括包含**CRYPTO**帧携带加密握手消息、ACK帧或两者都有。也可以包含**PING**, **PADDING**, 和**CONNECTION_CLOSE**帧。接收到包含其他帧的初始数据包的端点可以将该数据包作为伪数据丢弃，也可以将其视为连接错误。    
...
此数据包的有效负载包含加密帧，并且可以包含**PING**、**PADDING**或**ACK**帧。握手数据包可能包含**CONNECTION_CLOSE**帧。端点必须将接收到的与其他帧的握手数据包视为连接错误。    
...
+ 保留位（R）：字节0的下两个位（掩码为0x18）被保留。这些位使用头保护进行保护（见[QUIC-TLS]第5.4节）。保护前包含的值必须设置为0。端点必须在移除封包和标头保护后，将接收到的这些位元具有非零值的封包，视为**PROTOCOL_VIOLATION**类型的连接错误。在删除头保护之后丢弃这样的包会使端点暴露于攻击之下（参见[QUIC-TLS]第9.3节）。
...
每个端点都单方面决定是否为连接启用或禁用自旋位。实现必须允许客户端和服务器的管理员全局或基于每个连接禁用自旋位。即使管理员未禁用自旋位，端点也必须禁用自旋位的使用，以随机选择至少每16个网络路径中的一个，或每16个连接ID中的一个。由于每个端点都独立地禁用自旋位，因此可以确保在大约八分之一的网络路径上禁用自旋位信号。    
当自旋位被禁用时，端点可以将自旋位设置为任何值，并且必须忽略任何传入值。建议端点将自旋位设置为一个随机值，要么为每个包单独选择，要么为每个连接ID单独选择。    
如果连接启用了自旋位，则端点将维护自旋值，并在发送短头的数据包时将短头中的自旋位设置为当前存储的值。连接开始时端点中的自旋值初始化为0。每个端点还记录从连接上的对等端看到的最高包数。    
当服务器从客户机接收到增加服务器得到最高包数的短头数据包时，它将自旋值设置为等于接收到的数据包中的自旋位。    
当客户机从服务器接收到一个增加客户机得到最高数据包数的短头数据包时，它将自旋值设置为接收到的数据包中自旋位的倒数。    
当使用新的连接ID发送给定连接的第一个包时，端点将其自旋值重置为零。这降低了瞬态自旋位状态可用于跨连接迁移或ID更改链接流的风险。    
...
+ max_idle_timeout (0x01): 最大空闲超时是一个以毫秒为单位的值，编码为整数；见（第10.2节）。当两个端点都忽略此传输参数或指定值0时，将禁用空闲超时。
...
+ max_packet_size (0x03): 参数是一个整数值，它限制端点愿意接收的数据包的大小。这表示超过此限制的数据包将被丢弃。此参数的默认值是允许的最大UDP负载65527。低于1200的值无效。此限制仅适用于受保护的数据包（第12.1节）。
+ initial_max_data (0x04): 参数是一个整数值，它包含连接上可以发送的最大数据量的初始值。这相当于在完成握手后立即为连接发送MAX_DATA帧（第19.9节）。
+ initial_max_stream_data_bidi_local (0x05): 此参数是一个整数值，指定本地启动的双向流的初始流控制限制。此限制适用于由发送传输参数的端点打开的新创建的双向流。在客户端传输参数中，这适用于标识符最低有效两位设置为0x0的流；在服务器传输参数中，这适用于最低有效位设置为0x1的流。
+ initial_max_stream_data_bidi_remote (0x06): 此参数是一个整数值，指定对等启动的双向流的初始流控制限制。此限制适用于由接收传输参数的端点打开的新创建的双向流。在客户端传输参数中，这适用于标识符的最低有效位设置为0x1的流；在服务器传输参数中，这适用于最低有效两位设置为0x0的流。
+ initial_max_stream_data_uni (0x07): 此参数是一个整数值，指定单向流的初始流量控制限制。此限制适用于由接收传输参数的端点打开的新创建的单向流。在客户端传输参数中，这适用于标识符的最低有效位设置为0x3的流；在服务器传输参数中，这适用于最低有效两位设置为0x2的流。
...
+ max_ack_delay (0x0b): 最大确认延迟是一个整数值，表示端点延迟传送确认的最长时间（毫秒）。该值应包括接收器在警报触发时的预期延迟。例如：如果接收器将计时器设置为5毫秒，而警报通常延迟1毫秒，则它应发送最大6毫秒的确认延迟。如果没有此值，则假定默认值为25毫秒。比2^14大的值无效。
+ disable_active_migration (0x0c): 如果端点不支持活动连接迁移，则包含此参数（第9节）。设置此传输参数的端点的对等点不得从用于执行握手的本地地址或端口发送任何数据包，包括探测数据包（第9.1节）。此参数为零长度值。
...
+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
                        图 18: 首选地址格式
```
+ active_connection_id_limit (0x0e):  活动连接ID限制是一个整数值，指定端点愿意存储的对等端的最大连接ID数。该值包括在握手过程中接收到的连接ID、在首选地址传输参数中接收的连接ID以及在**NEW_CONNECTION_ID**帧中接收的连接ID。除非使用零长度的连接ID，否则"active_connection_ID_limit"参数的值必须不小于2。如果不存在此传输参数，则假定默认值为2。当使用长度为零的连接标识时，不能发送活动的连接标识限制参数。
...
### 19.2 PING帧
端点可以使用**PING**帧（type=0x01）来验证其对等方是否仍然有效，或者检查对等方的可访问性。PING帧不包含其他字段。    
**PING**帧的接收器只需确认包含该帧的包。   
当应用程序或应用协议希望防止连接超时时，**PING**帧可用于保持连接活动。应用程序协议应提供有关建议生成**PING**的条件的指导。此指南应指明预期发送**PING**的是客户端还是服务器。让两个端点在没有协调的情况下发送**PING**帧可能会产生过多的数据包和较差的性能。     
...
如果任何计算的数据包数为负数，则端点必须生成**FRAME_ENCODING_ERROR**类型的连接错误。     
